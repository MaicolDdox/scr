...
<div class="max-w-6xl mx-auto bg-white rounded-xl shadow-md p-6 animate-fade-in">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('HazardousWaste.create') }}" 
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">Nombre</th>
                    <th class="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase">Caracteristica</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($HazardousWastes as $entry)
                    <tr>
                        <td class="px-4 py-2 text-sm text-text-secondary">{{ $entry->date }}</td>
                        <td class="px-4 py-2 text-sm text-text-secondary">{{ $entry->generating_unit }}</td>
                        <td class="px-4 py-2 text-sm text-text-secondary">{{ $entry->waste_name }}</td>
                        <td class="px-4 py-2 text-sm text-text-secondary">{{ $entry->hazard_characteristic }}</td>
                        ...
                        <td class="px-4 py-2 text-sm text-text-secondary flex space-x-2">
                            <a href="{{ route('HazardousWaste.edit', $entry->id) }}" 
                               class="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition">Editar</a>
                            <form action="{{ route('HazardousWaste.destroy', $entry->id) }}" method="POST" onsubmit="return confirm('¿Está seguro de eliminar este registro?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
